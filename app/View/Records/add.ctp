<div>
    ...
    <?php 
        echo '<h2 style="margin-bottom: 5px;">' . __('Add new record') . '</h2>';
    ?>
    </div>
</div>
...
<?php 
	...
	if( isset($recs['Composition']) ) {
		...
		// Display
		echo $this->Form->input('x_composition_id', array('type' => 'text', 'readonly' => 'readonly', 'label' => false, 'value' => $recs['Composition'][0]['name'], 'after' => $this->Icon->trash('Composition', $recs['Composition'][0]['id'], '/Records/trash/Composition/' . $recs['Composition'][0]['id'])));
	} else {
	...
	if( isset($recs['Composer']) ) {
		...
		foreach($reccomposers as $key=>$val) {
			...
			// Display
			echo $this->Form->input('x_composer_id_.' . $no, array('type' => 'text', 'label' => false, 'value' => $val['name'], 'after' => $this->Icon->trash('Composer', $val['id'], '/Records/trash/Composer/' . $val['id'])));
			$no++;
		}
	}
	...
	if( isset($recs['Choir']) ) {
		...
		foreach($recchoirs as $key=>$val) {
			...
			// Display
			echo $this->Form->input('x_choir_id_.' . $no, array('type' => 'text', 'label' => false, 'value' => $val['name'], 'after' => $this->Icon->trash('Choir', $val['id'], '/Records/trash/Choir/' . $val['id'])));
			$no++;
		}
	}
	...
	if( isset($recs['Director']) ) {	
		...
		foreach($recdirectors as $key=>$val) {
			...
			// Display
			echo $this->Form->input('x_director_id_.' . $no, array('type' => 'text', 'label' => false, 'value' => $val['name'], 'after' => $this->Icon->trash('Director', $val['id'], '/Records/trash/Director/' . $val['id'])));
			$no++;
		}
	}
